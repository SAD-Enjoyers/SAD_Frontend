...
const RatingComponent = ({ serviceId }) => {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        console.log("Fetched data:", result); // کنسول لاگ برای بررسی داده‌ها
        ...
        if (result.status === 'success') {
          ...
          if (savedRating) {
            console.log("User score from localStorage:", savedRating);
            setUserScore(parseFloat(savedRating)); // اگر رای ذخیره شده بود، آن را در state قرار بده
          } else {
            console.log("User score from API:", courseData.userScore);
...
//         // استفاده از type و serviceId برای ساخت URL
//         let url = '';
//         if (type === 'course') {
//           url = `/api/v1/course/preview/${serviceId}`;
//         } else if (type === 'article') {
//           url = `/api/v1/article/preview/${serviceId}`;
//         } else if (type === 'exam') {
//           url = `/api/v1/exam/${serviceId}`;  // برای exam تغییر مسیر
//         }
...
//         const result = await response.json();
//         if (result.status === 'success') {
//           const fetchedData = type === 'course'
//             ? result.data.Course
//             : type === 'article'
//             ? result.data.Article
//             : result.data.Exam;  // برای exam داده‌ها در result.data.Exam قرار دارد

//           setData({
//             rating: parseFloat(fetchedData.score) || 0,  // میانگین امتیاز
//             ratingCount: fetchedData.numberOfVoters || 0,  // تعداد رای‌دهندگان
//           });
...
//             setUserScore(parseFloat(savedRating)); // اگر رای ذخیره شده بود، آن را در state قرار بده
//           } else {
//             setUserScore(fetchedData.userScore || 0); // اگر رای ذخیره‌شده نباشد، از داده‌های API استفاده کن
//           }
//         } else {
//           throw new Error('Unexpected response format');
//         }
...
//     fetchData(); // بارگذاری داده‌ها از API
//   }, [serviceId, type]);
...
//         // بررسی اینکه آیا کاربر قبلاً رای داده است یا خیر
//         const savedRating = localStorage.getItem(`rating-${serviceId}`);
//         const isNewRating = savedRating === null;
...
//             ...prevData,
//             rating: parseFloat(result.data.score),
//             ratingCount:  isNewRating ? prevData.ratingCount + 1 : prevData.ratingCount,
//           }));
...
//       <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bold', fontSize: '18px', color: '#000', marginBottom: '10px' }}>
//         Rate this {type === 'course' ? 'Course' : type === 'article' ? 'Article' : 'Exam'}
//       </Typography>
