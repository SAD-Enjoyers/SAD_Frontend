...
function ExamPreview() {
  const serviceId = 1; // Set serviceId to 1
  ...
  useEffect(() => {
    ...
    fetchExamData();
  }, [serviceId]); // Runs fetch when serviceId changes
  ...
  return (
    examData && (
      <div>
        ...
        <p>Price: {examData.price}</p>
        <p>Score: {examData.score}</p>
        <p>
          Tags: {examData.tag1}, {examData.tag2}, {examData.tag3}
        </p>
        <p>Number of Questions: {examData.numberOfQuestion}</p>
        <img src={examData.image} alt="Exam" style={{ maxWidth: "100%" }} />
      </div>
    )
  );
}
